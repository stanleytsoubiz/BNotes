# BNotes SEO / Deployment QC

掃描日期：2026-05-19T02:06:27.933Z
檢查基準日：2026-05-19

## 摘要

- 文章總數：71
- 已發布文章：64
- 未來稿：7
- P0 部署阻擋：0
- P1 應優先修正：0
- P2 內容成熟度項目：80

## P0 部署阻擋

- 通過

## P1 應優先修正

- 通過

## P2 內容成熟度項目

- alishan-new-crop-w02: 導讀卡尚未完全標準化
- alishan-terroir: 導讀卡尚未完全標準化
- alishan-terroir: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- bala-2026-world-latte-art-champion-taiwan: FAQ 尚未使用折疊式呈現
- bala-2026-world-latte-art-champion-taiwan: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- brewing-diagnosis-w01: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- canephora-flavor-wheel-guide: FAQ 尚未使用折疊式呈現
- canephora-flavor-wheel-guide: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- chiayi-coffee-festival-2026-guide: 導讀卡尚未完全標準化
- coffee-acidity-complete-science: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- coffee-dessert-pairing-science: 導讀卡尚未完全標準化
- coffee-reading-afternoon-ritual: 導讀卡尚未完全標準化
- cold-brew-advanced-science: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- cold-brew-complete-101: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- cold-brew-vs-ice-drip-vs-iced-americano: FAQ 尚未使用折疊式呈現
- cold-brew-vs-ice-drip-vs-iced-americano: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- colombia-main-harvest-selection: 導讀卡尚未完全標準化
- colombia-main-harvest-selection: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- colombia-mitaca-second-harvest: 導讀卡尚未完全標準化
- competition-recipe-homebrew-guide: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- cup-tasters-palate-training-guide: 導讀卡尚未完全標準化
- cup-tasters-palate-training-guide: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- espresso-ratio-science: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- ethiopia-natural-process-deep-dive: 導讀卡尚未完全標準化
- ethiopia-natural-process-deep-dive: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- ethiopia-vs-colombia-natural-showdown: 導讀卡尚未完全標準化
- ethiopia-vs-colombia-natural-showdown: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- flavor-pairing-w03: 導讀卡尚未完全標準化
- geisha-coffee-science-2026: FAQ 尚未使用折疊式呈現
- geisha-molecular-biology-2026: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- green-bean-sourcing-complete-guide: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- grinder-6models-comparison-2026: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- grinder-guide-2026: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- home-coffee-corner: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- home-roasting-beginners-complete: 導讀卡尚未完全標準化
- home-roasting-beginners-complete: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- home-roasting-beginners-guide-2026: FAQ 尚未使用折疊式呈現
- home-roasting-beginners-guide-2026: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- kenya-aa-flavor-profile-deep: 導讀卡尚未完全標準化
- kenya-aa-flavor-profile-deep: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- latte-art: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- light-roast-beginners-guide: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- milk-alternatives-science-2026: FAQ 尚未使用折疊式呈現
- milk-alternatives-science-2026: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- milk-steaming-science: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- morning-coffee-ritual-science: 導讀卡尚未完全標準化
- nitro-cold-brew-home-guide: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- plant-milk-barista-complete-guide: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- pour-over-variable-complete-experiment: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- sca-tasting-vocabulary-guide: 導讀卡尚未完全標準化
- sca-tasting-vocabulary-guide: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- single-origin-terroir-science: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- solo-brewing-valentines: 導讀卡尚未完全標準化
- solo-brewing-valentines: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- specialty-coffee-third-wave-history: 導讀卡尚未完全標準化
- specialty-coffee-third-wave-history: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- summer-iced-coffee-three-recipes: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- sustainable-coffee-complete-guide: 導讀卡尚未完全標準化
- sustainable-coffee-complete-guide: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- taipei-coffee-expo-2026-complete-guide: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- taipei-coffee-map-2026: FAQ 尚未使用折疊式呈現
- taipei-coffee-map-2026: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- taiwan-barista-2026-annual-review: 導讀卡尚未完全標準化
- taiwan-coffee-2026-new-season-terroir-guide: 導讀卡尚未完全標準化
- taiwan-coffee-2026-new-season-terroir-guide: 參考資料區尚未完全標準化
- taiwan-coffee-2026-new-season-terroir-guide: FAQ 尚未使用折疊式呈現
- taiwan-coffee-equipment-midyear-2026: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- taiwan-coffee-subscription-guide-2026: 導讀卡尚未完全標準化
- taiwan-specialty-coffee-whitepaper-2026: 導讀卡尚未完全標準化
- turkish-coffee-ibrik: 導讀卡尚未完全標準化
- v60-vs-kalita-filter-geometry: 導讀卡尚未完全標準化
- v60-vs-kalita-filter-geometry: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- wla-champion-2026-tech-breakdown: 導讀卡尚未完全標準化
- wlac-2026-champion-technique: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- world-coffee-weekly-vol1: 導讀卡尚未完全標準化
- world-coffee-weekly-vol1: 參考資料區尚未完全標準化
- world-coffee-weekly-vol1: FAQ 尚未使用折疊式呈現
- world-coffee-weekly-vol1: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting
- yunlin-gukeng-coffee-farmer-story: 導讀卡尚未完全標準化
- yunlin-gukeng-coffee-farmer-story: 長青文章仍使用 NewsArticle，建議改 Article 或 BlogPosting

