...
function renderPublished() {
  ...
  articlesList.innerHTML = f.map(a=>{
    ...
    if (ql) {
      ...
      if (ql.hasAffiliate) {
        qBtn += '<span class="affiliate-status-icon" title="偵測到聯盟連結，揭露模組已啟動">💰</span>';
      }
    } else {
  ...
  schedList.innerHTML = f.map(function(a){
    ...
    if (ql) {
      qBadge = '<button class="quality-light-badge ' + ql.light + '" data-slug="' + a.slug + '" data-title="' + escHtml(a.title) + '" title="\u6aa2\u8996\u54c1\u8cea\u5831\u544a">' + ql.lightLabel + '</button>' +
               ...
               (ql.hasAffiliate ? '<span class="affiliate-status-icon" title="偵測到聯盟連結">💰</span>' : '');
    } else {
...
function renderQualityPanel(d) {
  ...
  const affiliateHtml = d.hasAffiliate ? 
    '<div class="qp-section affiliate-alert" style="background:#fff3e0;border:1px solid #ffb74d;padding:.75rem;border-radius:.5rem;margin-bottom:1rem">' +
      '<div style="font-weight:700;color:#e65100;margin-bottom:.3rem">💰 聯盟行銷偵測</div>' +
      '<div style="font-size:.82rem;color:#5d4037">本文包含聯盟連結，前端「揭露模組」將自動於發布後注入聲明。</div>' +
    '</div>' : '';
