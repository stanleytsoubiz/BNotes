...
function renderPublished() {
  ...
  articlesList.innerHTML = f.map(a=>{
    ...
    const thumbHtml = '<div class="col-thumb"><img class="article-thumb" src="' + imgSrc + '" alt="" loading="lazy" onerror="this.parentNode.innerHTML=\'<div class=\"thumb-placeholder\">☕</div>\'"></div>';
...
function renderScheduled() {
  ...
  schedList.innerHTML = f.map(function(a){
    ...
    const sThumb = '<div class="col-thumb"><img class="article-thumb" src="' + sImg + '" alt="" loading="lazy" onerror="this.parentNode.innerHTML=\'<div class=\"thumb-placeholder\">☕</div>\'"></div>';
...
document.getElementById('btn-batch-publish')?.addEventListener('click',()=>{
  const cnt=scheduled.filter(a=>a.overdue).length;
  if(!cnt){showToast('目前沒有逾期文章','info');return;}
  document.getElementById('modal-batch-msg').innerHTML='確認批次發布 <strong>'+cnt+' 篇</strong>逾期文章？<br><small>日期將更新為今天。</small>';
  document.getElementById('modal-batch').classList.add('open');
});
document.getElementById('btn-batch-cancel')?.addEventListener('click',()=>document.getElementById('modal-batch').classList.remove('open'));
document.getElementById('btn-batch-confirm')?.addEventListener('click',async()=>{
  ...
  for(const slug of slugs){
    ...
    try{const r=await fetch('/api/publish-now',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({slug})});
    const d=await r.json();if(d.ok)done++;else fail++;}catch{fail++;}
    await new Promise(r=>setTimeout(r,800));
  }
  hideLoading();
  showToast('批次發布：成功'+done+'篇，失敗'+fail+'篇',done>0?'success':'error',6000);
  await loadScheduled(); setTimeout(loadPublished,2000);
});

// ── Stats Panel ───────────────────────────────────────────────────────────
document.getElementById('btn-stats')?.addEventListener('click',()=>{
  const catStats={};
  [...articles,...scheduled].forEach(a=>{const c=a.cat||a.category||'other';catStats[c]=(catStats[c]||0)+1;});
  const rows=Object.entries(catStats).sort((a,b)=>b[1]-a[1])
    .map(([c,n])=>'<div style="display:flex;justify-content:space-between;padding:.3rem 0;border-bottom:1px solid var(--border)"><span>'+(CAT_LABELS[c]||c)+'</span><span style="font-weight:700;color:var(--gold)">'+n+'</span></div>').join('');
  const overList=scheduled.filter(a=>a.overdue).slice(0,5)
    .map(a=>'<div style="font-size:.78rem;padding:.2rem 0;color:#d93025">● '+escHtml(a.title.substring(0,36))+'… ('+a.date+')</div>').join('');
  document.getElementById('stats-content').innerHTML=
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem;margin-bottom:1.2rem">'
    +'<div style="background:var(--muted);border-radius:.5rem;padding:.75rem;text-align:center"><div style="font-size:1.8rem;font-weight:900;color:var(--gold)">'+articles.length+'</div><div style="font-size:.78rem;color:var(--muted-fg)">已發布</div></div>'
    +'<div style="background:var(--muted);border-radius:.5rem;padding:.75rem;text-align:center"><div style="font-size:1.8rem;font-weight:900;color:var(--gold)">'+scheduled.length+'</div><div style="font-size:.78rem;color:var(--muted-fg)">排程中</div></div>'
    +'<div style="background:var(--muted);border-radius:.5rem;padding:.75rem;text-align:center"><div style="font-size:1.8rem;font-weight:900;color:#d93025">'+scheduled.filter(a=>a.overdue).length+'</div><div style="font-size:.78rem;color:var(--muted-fg)">逾期</div></div>'
    +'<div style="background:var(--muted);border-radius:.5rem;padding:.75rem;text-align:center"><div style="font-size:1.8rem;font-weight:900">'+aiPending.length+'</div><div style="font-size:.78rem;color:var(--muted-fg)">AI待審</div></div>'
    +'</div>'
    +'<h4 style="margin-bottom:.5rem;font-size:.85rem;font-weight:700">各分類分佈</h4>'+rows
    +(overList?'<h4 style="margin:.75rem 0 .4rem;font-size:.85rem;color:#d93025">⚠️ 逾期文章</h4>'+overList:'');
  document.getElementById('modal-stats').classList.add('open');
});
document.getElementById('btn-stats-close')?.addEventListener('click',()=>document.getElementById('modal-stats').classList.remove('open'));
